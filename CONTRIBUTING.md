# Contributing to ScheduledTasks.Linux.Native

## What this module is

Linux `*-ScheduledTask` cmdlets via `systemctl` subprocess and file I/O. 13 cmdlets + 2 stubs.
...
```bash
# Clone
git clone https://github.com/peppekerstens/ScheduledTasks.Linux.Native.git
cd ScheduledTasks.Linux.Native
...
1. Find an issue labeled [`good first issue`](https://github.com/peppekerstens/ScheduledTasks.Linux.Native/labels/good%20first%20issue)
...
```
fix: resolve pipe deadlock in SystemdHelpers.Run()
feat: add TaskRunLevel enum
docs: update STATUS.md for v1.0.0
test: add elevation error tests for write cmdlets
ci: add pr-validation.yml workflow
refactor: replace string TriggerType with TaskTriggerType enum
```
...
| Prefix | Example |
|---|---|
| `fix/` | `fix/issue-62-platform-branching` |
...
- **Bugs:** Use the [Bug Report](https://github.com/peppekerstens/ScheduledTasks.Linux.Native/issues/new?template=bug-report.md) template
- **Features:** Use the [Feature Request](https://github.com/peppekerstens/ScheduledTasks.Linux.Native/issues/new?template=feature-request.md) template
- **Code review findings:** Use the [Code Review](https://github.com/peppekerstens/ScheduledTasks.Linux.Native/issues/new?template=code-review.md) template
